...
function Comparator() {
  function getSizes(r: number, d: number): [number, number] {
    let step = (r * Math.PI) / HYPOTROCHOID_FIXED_RADIUS
  ...
  const sizes = Array.from({ length: 120 }, (_, i) => i + 1).map((r) => getSizes(r, distance))
  const sizes0 = sizes.map((s, i) => ({ x: i + 1, y: s[0] }))
  const sizes1 = sizes.map((s, i) => ({ x: i + 1, y: s[1] }))

  const evaluated0Points = Array.from({ length: 120 }, (_, i) => i + 1).map((r) => {
    const innerDataB = r < 60 ? polinomicFunction(bCoef, r) : polinomicFunction(mCoef, 120 - r)
    const innerDataM = r < 60 ? polinomicFunction(mCoef, r) : polinomicFunction(bCoef, 120 - r)
    return {
      x: r,
  ...
  const evaluated1Points = Array.from({ length: 120 }, (_, i) => i + 1).map((r) => {
    const innerDataB = r < 60 ? polinomicFunction(bCoef, r) : polinomicFunction(mCoef, 120 - r)
    const innerDataM = r < 60 ? polinomicFunction(mCoef, r) : polinomicFunction(bCoef, 120 - r)
    return {
      x: r,
