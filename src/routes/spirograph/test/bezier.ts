...
export function fitQuadraticBezier(points: Point[]): [Point, Point, Point] {
  if (points.length < 3) {
      throw new Error("Se necesitan al menos tres puntos para ajustar una curva de Bézier cuadrática.");
  }
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    const meanSquareError = points.reduce((sum, point, j) => {
      const t = tValues[j];
      const oneMinusT = 1 - t;
      const x = oneMinusT * oneMinusT * P0.x + 2 * oneMinusT * t * x1 + t * t * P2.x;
      const y = oneMinusT * oneMinusT * P0.y + 2 * oneMinusT * t * y1 + t * t * P2.y;
...
export function fitCubicBezier(points: Point[]): [Point, Point, Point, Point] {
  if (points.length < 4) {
      throw new Error("Se necesitan al menos cuatro puntos para ajustar una curva de Bézier cubica.");
  }

  function getCP(P0: number, Bt1: number, Bt2: number, P3: number, t1: number, t2: number): [number, number] {
    const oneMinusT1 = 1 - t1;
    const oneMinusT2 = 1 - t2;
    const numeratorT1 = (Bt1 - Math.pow(oneMinusT1, 3) * P0 - Math.pow(t1, 3) * P3) / (3 * t1 * t1 * oneMinusT1);
    const numeratorT2 = (Bt2 - Math.pow(oneMinusT2, 3) * P0 - Math.pow(t2, 3) * P3) / (3 * t2 * t2 * oneMinusT2);
  ...
  for (let i = 1; i < points.length - 2; i++) {
    for (let j = i + 1; j < points.length - 1; j++) {
      const [x1, x2] = getCP(P0.x, points[i].x, points[j].x, P3.x, tValues[i], tValues[j]);
      const [y1, y2] = getCP(P0.y, points[i].y, points[j].y, P3.y, tValues[i], tValues[j]);

      const meanSquareError = points.reduce((sum, point, k) => {
        const t = tValues[k];
        const oneMinusT = 1 - t;
        const x = Math.pow(oneMinusT, 3) * P0.x + 3 * oneMinusT * oneMinusT * t * x1 + 3 * oneMinusT * t * t * x2 + Math.pow(t, 3) * P3.x;
        const y = Math.pow(oneMinusT, 3) * P0.y + 3 * oneMinusT * oneMinusT * t * y1 + 3 * oneMinusT * t * t * y2 + Math.pow(t, 3) * P3.y;
        return sum + Math.hypot(point.x - x, point.y - y);
      }, 0) / points.length;
...
export function cubicBezier(points: number[], t: number): number {
  let oneMinusT = 1 - t;
  return Math.pow(oneMinusT, 3) * points[0] + 3 * oneMinusT * oneMinusT * t * points[1] + 3 * oneMinusT * t * t * points[2] + Math.pow(t, 3) * points[3];
}
