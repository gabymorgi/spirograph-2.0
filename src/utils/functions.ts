...
function SpiroLerp(pre: SpiroParams, post: SpiroParams, t: number): SpiroParams {
  return {
...
export function getSpiroTransition(pre: SpiroParams, post: SpiroParams, t: number): PathChunk[] {
  ...
  const calculateExtraParams = (isIncreasing: boolean) => {
    ...
    return {
      ...
      maxRadius: isClockwise
        ? isIncreasing ? lerp(pre.minRadius, 1, t) : lerp(1, post.minRadius, t)
        : isIncreasing ? lerp(-pre.minRadius, 1, t) : lerp(1, -post.minRadius, t),
      minSize: isIncreasing ? lerp(0, post.minSize, t) : lerp(pre.minSize, 0, t),
      maxSize: isIncreasing ? lerp(0, post.maxSize, t) : lerp(pre.maxSize, 0, t),
      stepAngle: isClockwise
        ? isIncreasing ? lerp(0, post.stepAngle, t) : lerp(pre.stepAngle, 0, t)
        : isIncreasing ? lerp(Math.PI, post.stepAngle, t) : lerp(pre.stepAngle, Math.PI, t),
      anglesAmount: Math.max(pre.anglesAmount, post.anglesAmount) - actParams.anglesAmount + 1,
    }
  };
  ...
  if (pre.anglesAmount !== post.anglesAmount) {
    ...
    const extraKeyPoints = getKeyPoints(extraParams, (actParams.anglesAmount - 1) * actParams.stepAngle);
    keyPoints.pop();
...
export function getTransitionDuration(pre: requiredSpiroParams, post: requiredSpiroParams): number {
  ...
  if (pre.petals !== post.petals) {
    if (pre.laps < pre.petals / 2 || post.laps < post.petals / 2) {
      ...
      duration = Math.max(duration, Math.abs(pre.petals - pre.laps - post.petals + post.laps))
    }
